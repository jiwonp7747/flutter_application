...
class _BoardInputPageState extends State<BoardInputPage> {
  ...
  List<XFile?> _images = List<XFile?>.filled(3, null); //파일 

  // 웹에서 이미지를 선택할 때 사용할 바이트 스트림 리스트 
  List<Uint8List?> _webImages = List<Uint8List?>.filled(3, null); //바이트스트림 
  ...
  Future<void> _takePhoto(int index) async {
    ...
    if (kIsWeb) { // 웹 환경인지 체크, 웹 환경에서는 카메라를 지원하지 않음 
  ...
  Future<void> _uploadData() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (kIsWeb) {
        // 웹에서 이미지를 업로드 할 때 바이트 스트림으로 변환하여 전송
        int i = 0;
